defmodule Explorer.SmartContract.Verifier do
  ...
  def evaluate_authenticity(_, %{"name" => "", "code" => ""}) do
    {:error, %{name: "can't be blank", code: "can't be"}}
  end

  def evaluate_authenticity(address_hash, %{"name" => name, "code" => code}) do
    solc_output = CodeCompiler.run(name, code)["contracts"][name][name]
